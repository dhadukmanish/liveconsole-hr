...
| A report PDF is blank or fails | It is generated on the host, so check `logs/`. A name in a script the standard PDF font cannot draw is already handled, but an unexpected failure shows there |
